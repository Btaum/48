...
context.__app.chart = { coin: 'BTCUSD', timeframe: '5m', tab: 'Long', data: {
  ...
  indicators: { ema15: series, ema1h: series.map(v => v - 120), macdLine: series.map((_, i) => Math.sin(i / 7) * 120), macdSignal: series.map((_, i) => Math.sin((i - 4) / 7) * 100), macdHist: series.map((_, i) => Math.sin(i / 5) * 60) },
  plan: { active: true, side: 'LONG', entry: 73300, sl: 72900, tp1: 73700, tp2: 74100, rr: '1:2' },
  signal: { entrySide: 'LONG', ema15: 73300, ema1h: 73100, macdLine: -20, macdSignal: -25, conditions: { longTrend: true, zeroOk: true, priceLowerLow: true, macdHigherLow: true, histColorChange: true, crossover: true }, divergence: { first: { index: 30 }, second: { index: 45 } } }, chartOffset: 0
}, loading: false, error: '' };
const html = context.__chartPage();
assert.ok(html.includes('chart-svg-wrap'), 'Chart SVG wrapper did not render');
assert.ok(html.includes('EMA50 (15m MTF)'), '15m MTF EMA label missing');
assert.ok(html.includes('EMA50 (1h MTF)'), '1h MTF EMA label missing');
...
assert.ok(html.includes('Confirmed bullish MACD crossover'), 'Long condition panel missing');
console.log('PASS: frontend chart page renders SVG, MTF EMA, MACD, Entry/SL/TP and projection panel.');
