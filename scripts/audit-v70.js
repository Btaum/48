...
assert.equal(pkg.version, '7.0.0', 'package version must be 7.0.0');
...
assert.equal(settings.requireDivergenceForEntry, false, 'V70 practical mode must make divergence optional');
assert.equal(settings.zeroLineMode, 'soft', 'V70 practical mode must make zero-line soft');
assert.equal(settings.require5mVolumeFlow, true, '5m volume flow must default on');
assert.ok(settings.marketMemoryMinSimilarityPct >= 55 && settings.marketMemoryMinSimilarityPct <= 80, 'Market Memory similarity default out of V70 practical range');
...
console.log('PASS: V70 audit confirmed balanced 5m-only scalp execution, 15m/1h/1D context, previous-day S/R, volume flow, Market Memory cloud, MACD timing, optional divergence/zero-line confluence, live order id verification, stale sync controls, trade totals, UI fields, and defaults.');
