...
function settingsPage() {
  ...
  return `${assetManagerHtml()}<section class="settings-layout"><section class="panel"><div class="panel-title"><h2>Strategy Settings</h2><span class="pill pill-data">V69 5m scalp + MTF S/R</span></div><p class="warning-box"><b>V69:</b> 5m scalp execution only. 15m/1h/1D are context for EMA50 direction, support/resistance, previous-day candle and volume flow. Market Memory cloud + MACD timing + side-correct 5m closed candle must pass before entry. Limit/pullback-first; LIVE requires Delta order id + bracket SL/TP payload.</p><form id="strategySettingsForm" class="settings-grid">
